...
Use `check-windows-proof-set.ps1 -RequireFullLaptopProof` after the three laptop reports exist. It reuses the same report profiles, verifies the reports came from the same Windows machine, package directory, and package identity fingerprint, then prints `proof_set_ok=full-laptop`, so the final claim is one set-level proof instead of three unrelated command outputs.
