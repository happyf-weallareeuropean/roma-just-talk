...
Pass `-PreflightOnly` with `-WhisperCLI` and `-WhisperModel` to run just the packaged hotkey-delivery, microphone, and local-whisper setup checks before providing cloud credentials or starting full dictation proof. It writes `preflight-proof.json` under `-ProofDir` by default, archives the hotkey key-down/key-up, 16 kHz mono microphone, and local-whisper no-network output markers, validates it through `check-windows-proof-set.ps1 -RequireLaptopPreflight`, and prints `proof_set_ok=laptop-preflight`; pass `-PreflightReportPath` when you want that JSON proof in a specific handoff location. Full laptop proof writes the same preflight report before interactive dictation and includes it in the final set check.
