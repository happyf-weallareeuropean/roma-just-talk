...
The laptop proof runner calls the artifact wrapper three times: cloud dictation with hold-to-talk and paste, local whisper dictation with hold-to-talk and paste, and local whisper Notepad paste. It then calls `check-windows-proof-set.ps1 -RequireFullLaptopProof` over the three JSON reports. This is the preferred laptop handoff command because it reuses the proven scripts instead of adding another install, dictation, or paste path. By default it creates the startup shortcut in the current user's real Startup folder; pass `-StartupShortcutDir C:\tmp\roma-startup-proof` only when you need a non-login temp proof folder.
Before each interactive dictation proof, the runner prints `ACTION_REQUIRED`, `hold_hotkey=Ctrl+Shift+R`, and the hold timeout so the operator can focus a normal text target and capture pre-roll speech deliberately.
The runner must be executed on Windows and requires real local `whisper-cli.exe` plus ggml model file paths; the packaged mock backend remains CI-only.
The wrapper validates the packaged artifact and manifest, runs the packaged agent and proof-agent doctors, delegates install/config/shortcut work to `install-windows-agent.ps1`, then verifies the installed launcher with `-DoctorOnly`.
Pass `-ProofReportPath` to leave a JSON proof record with package/install paths, Windows version/user identity, package identity fingerprint, config output path, dictation/paste flags, non-secret transcription config fields, doctor permission-surface output, proof-agent source-surface output, packaged native doctor output for hotkey, hold-hook, paste, DPAPI, and miniaudio capture, packaged and installed listener smoke output, file existence/byte counts/hash fields for the agent, packaged proof agent, installed launcher, optional shortcuts, exact shortcut target/argument/config references, local whisper files, dictation WAV, runtime-written WAV path and duration, numeric pre-roll fields, non-empty raw and processed transcript lengths, hold keydown/key-up runtime markers, Notepad paste-file proof when `-RunNotepadPasteProof` runs, and the `RomaWindowsAgent dictate` runtime log when `-RunDictation` creates one. Run `check-windows-proof-report.ps1` afterward to fail fast if the expected mode, Windows platform/user, package identity, packaged and installed listener mode, installed listener config and agent paths, installed agent/launcher hash match, exact shortcut launcher/config arguments, cloud/local-whisper config, permission surface, proof-agent source surface, native doctor surface, hold-hook config and runtime keydown/key-up, install, shortcut/startup shortcut launch target, dictation WAV, positive duration and pre-roll, positive transcript lengths, Notepad file paste, or paste proof fields are missing.
...
Use `check-windows-proof-set.ps1 -RequireFullLaptopProof` after the three laptop reports exist. It reuses the same report profiles, verifies the reports came from the same Windows machine, Windows user, package directory, and package identity fingerprint, then prints `proof_set_ok=full-laptop`, so the final claim is one set-level proof instead of three unrelated command outputs.
...
- CI also runs `package-windows-agent.ps1`, requires Swift runtime DLLs in the artifact, packages the proof-only `RomaWhisperCLIMock.exe`, packages and smokes `RomaProofAgent.exe`, verifies the packaged `RomaWindowsAgent.exe` through `smoke-windows-agent.ps1`, smokes packaged listener mode with `listen --max-sessions 0`, asserts generated JSON config for both cloud endpoint/model and local whisper-cli modes, proves no-admin installs for both cloud/default and local whisper-cli config into temp directories, verifies the installed launcher with `-DoctorOnly`, creates a proof-only mock shortcut in a temp folder, creates a real local-whisper shortcut in a separate temp folder, verifies both shortcuts point at their smoked configs, records the install config and shortcut paths in `manifest.txt`, packages and executes `prove-windows-agent-artifact.ps1 -DoctorOnly`, validates the packaged listener smoke, proof-agent source-surface, and native-doctor reports, runs the wrapper through the artifact-local manifest-backed local-whisper mock install, normal shortcut proof, startup shortcut proof, and installed launcher listener smoke, writes and validates a JSON wrapper proof report, and uploads a `roma-windows-agent` artifact for laptop smoke tests.
