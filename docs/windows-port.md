...
Use `check-windows-proof-set.ps1 -RequireFullLaptopProof` after the preflight report and three interactive laptop reports exist. It reuses the same report profiles, verifies the reports came from the same proof session, Windows machine, Windows user, package directory, package identity fingerprint, and source repository/branch/commit/dirty provenance, requires `source_dirty=false`, then prints `proof_set_ok=full-laptop`, so the final claim is one set-level proof instead of unrelated command outputs.
