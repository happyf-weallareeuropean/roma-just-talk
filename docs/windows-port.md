...
Pass `-PreflightOnly` with `-WhisperCLI` and `-WhisperModel` to run just the packaged hotkey-delivery, microphone, and local-whisper setup checks before providing cloud credentials or starting full dictation proof.
