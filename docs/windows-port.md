...
powershell -ExecutionPolicy Bypass -File C:\tmp\roma-windows-agent\check-windows-proof-report.ps1 -ProofReportPath C:\tmp\roma-windows-agent-proof.json -ExpectedMode cloud -RequireInstall -RequireShortcut -RequireStartupShortcut -RequireHoldHook -RequireCloudConfig -RequireDictation -RequirePaste
...
powershell -ExecutionPolicy Bypass -File C:\tmp\roma-windows-agent\check-windows-proof-report.ps1 -ProofReportPath C:\tmp\roma-windows-agent-local-whisper-proof.json -ExpectedMode local-whisper -RequireInstall -RequireShortcut -RequireStartupShortcut -RequireHoldHook -RequireWhisperConfig -RequireDictation -RequirePaste
```
