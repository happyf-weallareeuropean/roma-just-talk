...
Use `-RequireProofProfile` for the normal proof modes so the checker expands a single profile into the required atomic assertions. Current profiles are `doctor-only`, `cloud-dictation`, `local-whisper-dictation`, `local-whisper-notepad-paste`, and `packaged-whisper-mock-install`.
