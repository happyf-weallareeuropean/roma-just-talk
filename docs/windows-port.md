...
Pass `-PreflightOnly` with `-WhisperCLI` and `-WhisperModel` to run just the packaged hotkey-delivery, microphone, and local-whisper setup checks before providing cloud credentials or starting full dictation proof. It writes `preflight-proof.json` under `-ProofDir` by default, validates it through `check-windows-proof-set.ps1 -RequireLaptopPreflight`, and prints `proof_set_ok=laptop-preflight`; pass `-PreflightReportPath` when you want that JSON proof in a specific handoff location.
...
Before the interactive runs, the laptop proof runner records `mic-preflight.wav` with the packaged `RomaProofAgent.exe miniaudio-record-proof` command and asserts the 16 kHz mono WAV has payload bytes. Pass `-MicPreflightSeconds 2` when you want a longer microphone-access check before the full laptop proof.
