...
The laptop proof runner calls the artifact wrapper three times: cloud dictation with hold-to-talk and paste, local whisper dictation with hold-to-talk and paste, and local whisper Notepad paste. It stamps all three reports with one `proof_session_id`, then calls `check-windows-proof-set.ps1 -RequireFullLaptopProof` over the three JSON reports. This is the preferred laptop handoff command because it reuses the proven scripts instead of adding another install, dictation, or paste path. By default it creates the startup shortcut in the current user's real Startup folder; pass `-StartupShortcutDir C:\tmp\roma-startup-proof` only when you need a non-login temp proof folder.
...
Use `check-windows-proof-set.ps1 -RequireFullLaptopProof` after the three laptop reports exist. It reuses the same report profiles, verifies the reports came from the same proof session, Windows machine, Windows user, package directory, package identity fingerprint, and source repository/branch/commit/dirty provenance, requires `source_dirty=false`, then prints `proof_set_ok=full-laptop`, so the final claim is one set-level proof instead of three unrelated command outputs.
