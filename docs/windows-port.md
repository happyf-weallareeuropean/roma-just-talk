...
For CI or artifact smoke only, pass `-UsePackagedWhisperMock` instead of explicit `-WhisperCLI` and `-WhisperModel`. The wrapper reads `whisper_cli_mock` and `output` from `manifest.txt`; laptop proof should still pass real whisper.cpp paths or a real cloud endpoint/model.
