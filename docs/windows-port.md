...
Pass `-ProofReportPath` to leave a JSON proof record with package/install paths, Windows version, config output path, dictation/paste flags, non-secret transcription config fields, doctor permission-surface output, file existence/byte counts for the agent, installed launcher, optional shortcuts, local whisper files, dictation WAV, numeric pre-roll/duration fields, and the `RomaWindowsAgent dictate` runtime log when `-RunDictation` creates one. Run `check-windows-proof-report.ps1` afterward to fail fast if the expected mode, Windows platform, cloud/local-whisper config, permission surface, hold-hook config, install, shortcut, startup shortcut, dictation WAV, positive pre-roll, processed-text output, or paste proof fields are missing.
